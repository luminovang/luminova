...
final class Luminova 
{
     ...
     *
     * @param bool $uriQueryParams Whether to include query parameters in the cache ID (default: true).
     *
     ...
     * $cacheIdWithoutQuery = Luminova::getCacheId(false); // ignores query string
     * ```
     */
    public static function getCacheId(bool $uriQueryParams = true): string 
    {
        $uriQueryParams = (bool) env('page.caching.uri.query.param', $uriQueryParams);
        $uri = ($_SERVER['REQUEST_URI'] ?? 'index');
        $id = ($_SERVER['REQUEST_METHOD'] ?? 'CLI');
        $id .= $uriQueryParams ? $uri : (parse_url($uri, PHP_URL_PATH) ?: '');
